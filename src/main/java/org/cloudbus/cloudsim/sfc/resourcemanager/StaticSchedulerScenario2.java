...
public class StaticSchedulerScenario2 extends DeploymentScheduler {
    ...
    public StaticSchedulerScenario2(List<ServiceFunctionChain> serverFunctionChains, List<SFCWorkload> sfcWorkloads,
    ...
    public void generateLinks(List<ServiceFunctionChain> serverFunctionChains) {
        generateLinksForScenario1(serverFunctionChains);
    }
    ...
    public void generateNodes(List<SFCWorkload> sfcWorkloads, List<Resource> resources,
        ...
        generateNodesForScenario1(sfcWorkloads, resources, serviceFunctionChains);
    }

    public void generateLinksForScenario1(List<ServiceFunctionChain> serverFunctionChains) {
    ...
    public void generateNodesForScenario1(List<SFCWorkload> sfcWorkloads, List<Resource> resources,
            List<ServiceFunctionChain> serviceFunctionChains) {
    ...
    public void generateLinkTCAware(List<ServiceFunctionChain> serverFunctionChains) {
        ...
        for (ServiceFunctionChain sfc : physicalChains.keySet()) {
            ...
            VirtualTopologyLink ingressLink = new VirtualTopologyLink(ingress.getName() + "-" + firstSF.getName(),
                    ingress.getName(), firstSF.getName(), requestSize);
            links.add(ingressLink);
            requestSize *= (int) (ServiceFunction.serverFunctionMap.get(firstSF.getType()).getOutputRate()
            ...
            for (int i = 0; i < serviceFunctions.size() - 1; i++) {
                ...
                requestSize *= ratio; // 方法2-- link的bandwidth从头到尾是变化的
            }
    ...
    public void generateLinkMaxBW(List<ServiceFunctionChain> serverFunctionChains) {
        ...
        for (ServiceFunctionChain sfc : physicalChains.keySet()) {
            ...
            VirtualTopologyLink ingressLink = new VirtualTopologyLink(
                    ingress.getName() + "-" + serviceFunctions.get(0).getName(), ingress.getName(),
                    serviceFunctions.get(0).getName(), maxBw);
            links.add(ingressLink);
            ...
            VirtualTopologyLink egressLink = new VirtualTopologyLink(
                    serviceFunctions.get(serviceFunctions.size() - 1).getName() + "-" + egress.getName(),
                    serviceFunctions.get(serviceFunctions.size() - 1).getName(), egress.getName(), maxBw);
            links.add(egressLink);
        }
    }
