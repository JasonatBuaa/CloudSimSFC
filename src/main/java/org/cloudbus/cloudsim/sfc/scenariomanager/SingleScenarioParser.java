...
//        this.serviceFunctionFileName = "Comparison_ServiceFunctionDescription.json";
        ...
        SingleScenarioParser.prepareFolder(this.rootPath);
        SingleScenarioParser.prepareFolder(this.workloadFolder);
        SingleScenarioParser.prepareFolder(this.compSDNNFVRootPath);
    }

    public SingleScenarioParser() {
        this.sfcWorkloads = new ArrayList<>();
        prepareSubFolders();
    }

    public SingleScenarioParser(String rootFolder, String subFolder) {
        this.sfcWorkloads = new ArrayList<>();
        this.rootPath = rootFolder + subFolder;
        SingleScenarioParser.prepareFolder(this.rootPath);

        this.workloadFolder = rootPath + "workloads/";
        SingleScenarioParser.prepareFolder(workloadFolder);

        this.resourceDescriptionFileName = rootFolder + "ResourceDesription.json";
        this.SFCDemandFileName = "SFCDemands.json";
        this.serviceFunctionFileName = "Comparison_ServiceFunctionDescription.json";
        this.compSDNNFVRootPath = this.rootPath + "sdnnfv/";
        SingleScenarioParser.prepareFolder(this.compSDNNFVRootPath);
...
//        sfc.compSetChain(chainLength, rootPath + serviceFunctionFileName);
    ...
    public void generateWorkload() {
        for (SFCWorkload sfcWorkload : sfcWorkloads) {
            ...
            // + ".csv", sfcWorkload);
            workloadsCsvWriter(workloadFolder + "workloads_large_" + sfcWorkload.getTargetChainName() + ".csv", sfcWorkload);
        }
    }
...
//            String folder = workloadFolder + "sdnnfv/";
            ...
            SingleScenarioParser.prepareFolder(folder);
...
//        String path = rootPath + resourceDescriptionFileName;
        String path = resourceDescriptionFileName;
...
//        String path = rootPath + serviceFunctionFileName;
        String path = serviceFunctionFileName;
        String jsonStr = JsonUtils.jsonRead(path);
