...
/**
 * A Host is a Physical Machine (PM) inside a Datacenter. It is also called as a
 * Server. It executes actions related to management of virtual machines (e.g.,
...
public class Host {
	...
	/**
	 * The Processing Elements (PEs) of the host, that represent the CPU cores of
	 * it, and thus, its processing capacity.
	 */
	 ...
	 *         host or {@link Double#MAX_VALUE} if there is no future events
	 *         expected in this host
	 * @pre currentTime >= 0.0
	 * @post $none
	 * @todo there is an inconsistency between the return value of this method and
	 *       the individual call of {@link Vm#updateVmProcessing(double,
	 *       java.util.List), and consequently the
	 *       {@link CloudletScheduler#updateVmProcessing(double, java.util.List)}.
	 *       The current method returns {@link Double#MAX_VALUE} while the other
	 *       ones return 0. It has to be checked if there is a reason for this
	 *       difference.}
	 */
	...
	/**
	 * Reallocate migrating in vms. Gets the VM in the migrating in queue and
	 * allocate them on the host.
	 */
	...
	/**
	 * Checks if the host is suitable for vm. If it has enough resources to attend
	 * the VM.
	 * 
	 * @param vm the vm
	...
	/**
	 * Sets the PEs of the host to a FAILED status. NOTE: <tt>resName</tt> is used
	 * for debugging purposes, which is <b>ON</b> by default. Use
