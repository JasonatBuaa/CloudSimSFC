...
/**
 * VmScheduler is an abstract class that represents the policy used by a Virtual
 * Machine Monitor (VMM) to share processing power of a PM among VMs running in
 * a host.
 * 
 * Each host has to use is own instance of a VmScheduler that will so schedule
 * the allocation of host's PEs for VMs running on it.
 * 
...
public abstract class VmScheduler {
	...
	/**
	 * The map of VMs to PEs, where each key is a VM id and each value is a list of
	 * PEs allocated to that VM.
	 */
	private Map<String, List<Pe>> peMap;

	/**
	 * The map of VMs to MIPS, were each key is a VM id and each value is the
	 * currently allocated MIPS from the respective PE to that VM. The PEs where the
	 * MIPS capacity is get are defined in the {@link #peMap}.
	 * 
	 ...
	 * 
	 * @param pelist the list of PEs of the host where the VmScheduler is associated
	 *               to.
	 * @pre peList != $null
	 * @post $none
	 */
	...
	/**
	 * Releases PEs allocated to a VM. After that, the PEs may be used on demand by
	 * other VMs.
	 * 
	 * @param vm the vm
	 * @pre $none
	...
	/**
	 * Releases PEs allocated to all the VMs of the host the VmScheduler is
	 ...
	 * 
	 * @return mips
	 * @todo It considers that all PEs have the same capacity, what has been shown
	 *       doesn't be assured. The peList received by the VmScheduler can be
	 *       heterogeneous PEs.
	 */
