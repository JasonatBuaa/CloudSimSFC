...
public class DeploymentFileParser {
    ...
    private Hashtable<String, Integer> parseVMs(JSONObject doc) {
        ...
        while (iter.hasNext()) {
            ...
            if (Configuration.USE_AGGREGATE_PES == true) {
                mips = mips != 0 ? (mips * pes) : 0;
                pes = pes != 0 ? 1 : 0;
            }
...
//				double doubleQueueSize = (Double) (node.get("queuesize"));
//				doubleQueueSize *= Configuration.QUEUE_SPACE_MULTIPLY;
//				queueSize = new BigDecimal(doubleQueueSize).longValue();
