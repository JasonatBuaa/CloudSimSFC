...
public class WorkloadResultWriter {
    ...
    /*
     * private void threadPrintWorkload(Workload wl) { thread.enqueue(wl); }
     *
     * private void threadExit() { thread.setExit(); try { Thread.sleep(1); } catch
    ...
    public void printWorkload(Workload wl) {
        ...
        if (wl.failed) {
            ...
            // Jason!！ The following part is just a walk-around for finding timeout flows!
            if(serveTime > Configuration.TIME_OUT)
            {
    ...
    // Jason: print result; change at will; !!!
...
//        printLine("Dropped cloudlet due to queue full: " + SDNBroker.dropedCloudletsDueToQueueFull);
    }
    ...
    public static double getWorkloadStartTime(Workload w) {
        ...
        for (Activity act : acts) {
//			Jason: different from CloudSimSDN-NFV
            if (act instanceof Transmission) {
