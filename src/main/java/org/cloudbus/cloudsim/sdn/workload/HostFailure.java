...
public class HostFailure implements Activity {
    ...
    public HostFailure(Cloudlet cl) {
        this.cl = cl;
    }
    ...
    /**
     * 解析和存储Cloudlet的数据，并存储起来；然后删除掉原有Cloudlet
     */
    ...
    private double getMaxMipsForCloudlet() {
         ...
         * 
         * if(cloudletLen < vmMipsPerPE * 0.5) { mipsPercent =
         * Configuration.CPU_REQUIRED_MIPS_PER_WORKLOAD_PERCENT[0]; } else
         * if(cloudletLen < vmMipsPerPE * 1.0) { mipsPercent =
         * Configuration.CPU_REQUIRED_MIPS_PER_WORKLOAD_PERCENT[1]; } else
         * if(cloudletLen < vmMipsPerPE * 1.5) { mipsPercent =
         * Configuration.CPU_REQUIRED_MIPS_PER_WORKLOAD_PERCENT[2]; } else { mipsPercent
         * = Configuration.CPU_REQUIRED_MIPS_PER_WORKLOAD_PERCENT[3]; }
         */
