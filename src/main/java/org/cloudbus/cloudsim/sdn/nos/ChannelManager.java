...
public class ChannelManager {
	...
	// Processing requests
	protected HashMap<String, Channel> channelTable = new HashMap<String, Channel>(); // getKey(fromVM, toVM, flowID) ->
																						// Channel
	...
	public Channel createChannel(int src, int dst, int flowId, Node srcNode) {
		...
		if (dest == null) {
			throw new IllegalArgumentException(
					"createChannel(): dest is null, cannot create channel! " + NetworkOperatingSystem.findVmGlobal(src)
		...
		while (true) {
			...
			// Log.printLine(CloudSim.clock() + ": createChannel() :("
			// +getKey(src,dst,flowId)+"): "+link);
		...
		if (flowId != -1 && lowestBw < reqBw) {
			// Cannot make channel.
			// Log.printLine(CloudSim.clock() + ": " + getName() + ": Free bandwidth is less
			// than required.("+getKey(src,dst,flowId)+"): ReqBW="+ reqBw + "/
			// Free="+lowestBw);
			// return null;
		}
	...
	public List<Channel> findAllChannels(int vmId) {
		...
		for (Channel ch : channelTable.values()) {
			if (ch.getSrcId() == vmId || ch.getDstId() == vmId) {
				allChannels.add(ch);
			}
		}
