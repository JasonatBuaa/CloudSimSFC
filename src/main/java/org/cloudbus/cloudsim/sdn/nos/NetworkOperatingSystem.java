...
public abstract class NetworkOperatingSystem extends SimEntity {
	 ...
	 * advise host, advise dc 2. set channels and bws 3. set routing tables to
	 * restrict hops to meet latency
	 * 
	 * @param sfcPolicy
	 */
	...
	public void processEvent(SimEvent ev) {
		...
		switch (tag) {
			case CloudSimTagsSDN.SDN_INTERNAL_CHANNEL_PROCESS:
				processInternalAdjustChannels(); // 当传输数据量（虚拟链路需要的带宽）小于物理链路的实际带宽时，则不能满足物理链路的带宽需求，此时将实际带宽分配给物理链路
				break;
			...
			case CloudSimTagsSDN.MONITOR_UPDATE_UTILIZATION:
				...
				if (CloudSimEx.hasMoreEvent(CloudSimTagsSDN.MONITOR_UPDATE_UTILIZATION)) {
					...
					// If there's no event between now and the next monitoring time, skip monitoring
	...
	protected void processVmCreateDynamicAck(SimEvent ev) {
		...
		if (result) {
			Log.printLine(CloudSim.clock() + ": " + getName() + ".processVmCreateDynamic: Dynamic VM(" + newVm
					+ ") creation succesful!");
	...
	public Packet addPacketToChannel(Packet orgPkt) {
		Packet pkt = orgPkt;
		/*
		 * if(sender.equals(sender.getVMRoute(src, dst, flowId))) { // For loopback
		 * packet (when src and dst is on the same host)
		 * //Log.printLine(CloudSim.clock() + ": " + getName() +
		 * ".addPacketToChannel: Loopback package: "+pkt +". Send to destination:"+dst);
		 * sendNow(sender.getAddress(),Constants.SDN_PACKAGE,pkt); return; }
		 */
		...
		// Log.printLine(CloudSim.clock() + ": " + getName() + ".addPacketToChannel
		// ("+channel
		// +"): Transmission added:" +
	...
	private void migrateChannel(Vm vm, SDNHost oldHost, SDNHost newHost) {
		for (Channel ch : channelManager.findAllChannels(vm.getId())) {
			...
			vnMapper.buildNodesLinks(ch.getSrcId(), ch.getDstId(), ch.getChId(), sender, nodes, links);
	...
	public void addExtraVm(SDNVm vm, NetworkOperatingSystem callback) {
		...
		Log.printLine(CloudSim.clock() + ": " + getName() + ": Add extra VM #" + vm.getId() + " in "
				+ datacenter.getName() + ", (" + vm.getStartTime() + "~" + vm.getFinishTime() + ")");
	...
	public void removeExtraVm(SDNVm vm) {
		...
		Log.printLine(CloudSim.clock() + ": " + getName() + ": Remove extra VM #" + vm.getId() + " in "
				+ datacenter.getName() + ", (" + vm.getStartTime() + "~" + vm.getFinishTime() + ")");
	...
	public void addExtraPath(int orgVmId, int newVmId) {
		...
		// This function finds all Flows involving orgVmId and add another virtual path
		// for newVmId.
	...
	public void updateVmMips(SDNVm orgVm, int newPe, double newMips) {
		...
		if (!this.datacenter.getVmAllocationPolicy().allocateHostForVm(orgVm, host)) {
			System.err.println("ERROR!! VM cannot be resized! " + orgVm + " (new Pe " + newPe + ", Mips " + newMips
					+ ") in host: " + host);
			System.exit(-1);
		}
	}
	 ...
	 * only
	 * 
	 * Collection<Link> links = this.topology.getAllLinks(); for(Link l:links) {
	 * System.err.println(l); MonitoringValues mv =
	 * l.getMonitoringValuesLinkUtilizationUp(); System.err.print(mv); mv =
	 * l.getMonitoringValuesLinkUtilizationDown(); System.err.print(mv); } // //
	 * for(Channel ch:this.allChannels) { // System.err.println(ch); //
	 * MonitoringValues mv = ch.getMonitoringValuesLinkUtilization(); //
	 * System.err.print(mv); // }
	 * 
	 * for(SDNHost h:datacenter.<SDNHost>getHostList()) { System.err.println(h);
	 * MonitoringValues mv = h.getMonitoringValuesHostCPUUtilization();
	 * System.err.print(mv); }
	 * 
	 * for(Vm vm:vmMapId2Vm.values()) { SDNVm tvm = (SDNVm)vm;
	 * System.err.println(tvm); MonitoringValues mv =
	 * tvm.getMonitoringValuesVmCPUUtilization(); System.err.print(mv); } }
	 */
}
