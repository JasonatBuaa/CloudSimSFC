...
public class VirtualTopologyGeneratorVmTypesEdge extends VirtualTopologyGeneratorVmTypes {
	...
	public void generateLarge3TierTopologyEdge(String jsonFileName, boolean noscale) {
		...
		final Long[] linkBW = new Long[] { 1500000L, 1500000L, 1500000L, 1500000L, 1500000L, 1500000L, 1500000L,
				1500000L, 1500000L, 1500000L, 1500000L, 1500000L, 1500000L, 1500000L, 1500000L };
	 ...
	 * @param noscale
	 */
	public void generateVMGroupComplex(int numWeb, int numApp, int numDB, TimeGen startTime, TimeGen endTime,
			Long linkBw, int groupId, boolean noscale) {
		...
		for (int i = 0; i < numWeb; i++)
			webs[i] = this.createVM(VMtype.WebServer, startTime.getStartTime(), endTime.getEndTime(), groupId, i,
					linkBw);

		// 为何app的starttime要隔着两个更新一次？
		// App
		...
		int maxNum = Integer.max(numWeb, numApp); // 三种负载当中数量最大的一个
		maxNum = Integer.max(maxNum, numDB);

		// Jason: noscale是什么？ linkBwPerCh是per channel的意思吗？
	...
	public void createSFCPolicy(VMSpec[] webs, VMSpec[] apps, VMSpec[] dbs, TimeGen startTime, TimeGen endTime,
		...
		/*
		 * SFSpec [] lb2s = new SFSpec[lb2Num]; for(int i=0; i<lb2Num; i++) { //lb2s[i]
		 * = addSFLoadBalancer("lb2"+i, linkBw, startTime, endTime, noscale); } SFSpec
		 * [] idss = new SFSpec[idsNum]; for(int i=0; i<idsNum; i++) { //idss[i] =
		 * addSFIntrusionDetectionSystem("ids"+i, linkBw, startTime, endTime, noscale);
		 * }
		 */
		...
		// Policy for App -> DB
		/*
		 * { List<SFSpec>[] chains = createSFCombination(lb2s, idss); double expTime =
		 * 1.0; addSFCPolicyCollective(apps, dbs, chains, expTime); }//
		 */

		// Policy for DB -> App
		/*
		 * { List<SFSpec>[] chains = createSFCombination(idss, lb2s); double expTime =
		 * 1.0; addSFCPolicyCollective(dbs, apps, chains, expTime); }//
		 */

		// Policy for App -> Web
		/*
		 * { List<SFSpec> chain = new ArrayList<SFSpec>(); chain.add(lb1s[0]);
		 * 
		 * @SuppressWarnings("unchecked") List<SFSpec>[] chains = new List[1]; chains[0]
		 * = chain; double expTime = 1.0; addSFCPolicyCollective(apps, webs, chains,
		 * expTime); }//
		 */
	}
	...
	public SFSpec addSFFirewall(String name, long linkBw, TimeGen startTime, TimeGen endTime, boolean noscale) {
		...
		SFSpec sf = addSF(name, datacenter, Arrays.asList(subdatacenter), pes, mips, ram, storage, bw,
				startTime.getStartTime(), endTime.getEndTime(), miPerOperation, "Firewall");
	...
	public SFSpec addSFLoadBalancer(String name, long linkBw, TimeGen startTime, TimeGen endTime, boolean noscale) {
		...
		SFSpec sf = addSF(name, datacenter, Arrays.asList(subdatacenter), pes, mips, ram, storage, bw,
				startTime.getStartTime(), endTime.getEndTime(), miPerOperation, "LoadBalancer");
	...
	public SFSpec addSFIntrusionDetectionSystem(String name, long linkBw, TimeGen startTime, TimeGen endTime,
		...
		SFSpec sf = addSF(name, datacenter, Arrays.asList(subdatacenter), pes, mips, ram, storage, bw,
				startTime.getStartTime(), endTime.getEndTime(), miPerOperation, "IDS");
