...
/**
 * CloudSimSDN example main program for Edge experiment. It loads physical
 * topology file, application deployment configuration file and workload files,
 * and run simulation. Simulation result will be saved into files.
 * 
 * @author Jungmin Son
...
public class StartExperimentSFCEdge {
	...
	public static void setExpName(String policy, String sfOn) {
		if (Configuration.SFC_LATENCY_AWARE_ENABLE) {
			Configuration.experimentName = String.format("SFC_On_%s_%d_%s_", sfOn,
					(int) Configuration.migrationTimeInterval, policy);
		} else {
			Configuration.experimentName = String.format("SFC_Off_%s_%d_%s_", sfOn,
					(int) Configuration.migrationTimeInterval, policy);
		}
	}
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		try {
			...
			Map<NetworkOperatingSystem, SDNDatacenter> dcs = createPhysicalTopology(physicalTopologyFile, ls,
					vmAllocationFac, hostSelectionPolicy, vmMigrationPolicy);
	...
	public static Map<NetworkOperatingSystem, SDNDatacenter> createPhysicalTopology(String physicalTopologyFile,
			LinkSelectionPolicy ls, VmAllocationPolicyFactory vmAllocationFac, HostSelectionPolicy hostSelectionPolicy,
			VmMigrationPolicy vmMigrationPolicy) {
	...
	protected static SDNDatacenter createSDNDatacenter(String name, NetworkOperatingSystem snos,
			VmAllocationPolicyFactory vmAllocationFactory, HostSelectionPolicy hostSelectionPolicy,
			VmMigrationPolicy vmMigrationPolicy) {
