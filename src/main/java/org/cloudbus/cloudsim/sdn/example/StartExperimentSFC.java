...
public class StartExperimentSFC {
	// protected static String physicalTopologyFile = "SmallSFC-physical.json";
	// protected static String deploymentFile = "SmallSFC.json";
	// protected static String[] workload_files = { "SmallSFCworkload.csv",
	...
	// protected static String workload_file = "SmallSFCworkload.csv";
	...
	protected static String workload_file = "SmallSFCDemo/sfc-example-workload-normal-user.csv";
	// protected static String workload_file = "SmallSFCDemo/smallworkload.csv";
	// protected static String[] workload_files = {
	// "SmallSFCDemo/sfc-example-workload-normal-user.csv" };
	protected static String[] workload_files = {};

	private static String[] argString = { "LFF", "0", physicalTopologyFile, deploymentFile, "./", workload_file };
	...
	enum VmAllocationPolicyEnum {
		LFF, MFF, HPF, MFFGroup, LFFFlow, MFFFlow, HPFFlow, MFFGroupFlow, Random, RandomFlow, MFFBW, MFFCPU, END
	}

	private static void printUsage() {
		String runCmd = "java SDNExample";
		System.out.format(
				"Usage: %s <LFF|MFF> <0|1> <physical.json> <virtual.json> <working_dir> [workload1.csv] [workload2.csv] [...]\n",
				runCmd);
	}
	...
	public static void setExpName(String policy, String sfOn) {
		if (Configuration.SFC_AUTOSCALE_ENABLE) {
			Configuration.experimentName = String.format("Demo_SFC_On_%s_%d_%s_", sfOn,
					(int) Configuration.migrationTimeInterval, policy);
		} else {
			Configuration.experimentName = String.format("Demo_SFC_Off_%s_%d_%s_", sfOn,
					(int) Configuration.migrationTimeInterval, policy);
		}
	}
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		// args = new String[] { "LFF", "0", "dataset-energy/energy-physical.json",
		// "dataset-energy/energy-virtual.json",
		// "./", "dataset-energy/energy-workload.csv" };

		args = argString;

		// <LFF|MFF> <0|1> <physical.json> <virtual.json> <working_dir> [workload1.csv]
		// [workload2.csv] [...]
		...
		// 1. Policy: MFF, LFF, ...
		String policy = args[n++]; // args[0]: policy

		String sfcOn = args[n++]; // args[1]: autoScaling
		if ("1".equals(sfcOn)) {
		...
		if (args.length > n)
			physicalTopologyFile = args[n++]; // args[2] topo <physical.json>

		// 3. Virtual Topology filename
		if (args.length > n)
			deploymentFile = args[n++]; // args[3] <virtual.json>
		...
		if (args.length > n) {
			...
			} else { // <working_dir> [workload1.csv] [workload2.csv] [...]\n
				int i = n;
				if (args.length > n + 1) {
		...
		FileOutputStream output = new FileOutputStream(
				Configuration.workingDirectory + "SmallSFCDemo/result/" + Configuration.experimentName + "log.out.txt");
		Log.setOutput(output);

		// Jason: Log file directory
