 ...
 */
public class VmSchedulerTimeSharedOverSubscriptionDynamicVM extends VmSchedulerTimeSharedOverSubscription
	...
	/**
	 * Check the number of cloudlets processing in each VM. Steal MIPS from idle VM
	 * to give to busy VMs
	 */
	...
	/*
	 * / Host host= null; public void debugSetHost(Host host) { this.host = host; }
	 * 
	 * private List<PowerUtilizationHistoryEntry> utilizationHistories = null;
	 * private static double powerOffDuration = 0; //if host is idle for 1 hours,
	 * it's turned off.
	 * 
	 * @Override protected void setAvailableMips(double availableMips) {
	 * super.setAvailableMips(availableMips); addUtilizationEntry(); }
	 * 
	 * public void addUtilizationEntryTermination(double terminatedTime) {
	 * if(this.utilizationHistories != null) this.utilizationHistories.add(new
	 * PowerUtilizationHistoryEntry(terminatedTime, 0)); }
	 * 
	 * public List<PowerUtilizationHistoryEntry> getUtilizationHisotry() { return
	 * utilizationHistories; }
	 * 
	 * public double getUtilizationEnergyConsumption() {
	 * 
	 * double total=0; double lastTime=0; double lastUtilPercentage=0;
	 * if(this.utilizationHistories == null) return 0;
	 * 
	 * for(PowerUtilizationHistoryEntry h:this.utilizationHistories) { double
	 * duration = h.startTime - lastTime; double utilPercentage =
	 * lastUtilPercentage; double power = calculatePower(utilPercentage); double
	 * energyConsumption = power * duration;
	 * 
	 * // Assume that the host is turned off when duration is long enough
	 * if(duration > powerOffDuration && lastUtilPercentage == 0) energyConsumption
	 * = 0;
	 * 
	 * //////////////////////////////// // DEBUG boolean debugPrint = false; if(host
	 * != null && host.getId() ==28) debugPrint = true; if(debugPrint &&
	 * energyConsumption != 0) {
	 * System.err.println("DEBUG!! from: "+lastTime+", to: "+h.
	 * startTime+", utilization: "+utilPercentage+", power: "+energyConsumption/3600
	 * ); } ///////////////////////////////
	 * 
	 * total += energyConsumption; lastTime = h.startTime; lastUtilPercentage =
	 * h.utilPercentage; } return total/3600; // transform to Whatt*hour from
	 * What*seconds }
	 * 
	 * private double calculatePower(double u) { double power = 120 + 154 * u;
	 * return power; }
	 * 
	 * private void addUtilizationEntry() { double time = CloudSim.clock(); double
	 * totalMips = getTotalMips(); double usingMips = totalMips -
	 * this.getAvailableMips(); if(usingMips < 0) {
	 * System.err.println("addUtilizationEntry : using mips is negative, No way!");
	 * } if(utilizationHistories == null) utilizationHistories = new
	 * ArrayList<PowerUtilizationHistoryEntry>(); this.utilizationHistories.add(new
	 * PowerUtilizationHistoryEntry(time, usingMips/ getTotalMips())); }
	 * 
	 * private double getTotalMips() { return this.getPeList().size() *
	 * this.getPeCapacity(); } //
	 */
}
