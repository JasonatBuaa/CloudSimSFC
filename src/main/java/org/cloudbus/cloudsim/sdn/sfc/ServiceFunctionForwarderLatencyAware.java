 ...
 * Forwarding rule: Latency-aware forwarding which gives priority to the
 * delay-sensitive SFC policy. If a SF is shared by several SFCs with different
 * latency requirement, packets for lower latency requirement will be forwarded
 * to the nearest SF from the source.
 *
 * @author Jungmin Jay Son, Jason Sun
 * @since CloudSimSDN 3.0
 */
public class ServiceFunctionForwarderLatencyAware extends ServiceFunctionForwarder {
	/**
	 * A map to store all SFC policies passing through a specific SF. Key: VM ID of
	 * the SF Value: List of SFCPolicy which contains the SF of the key
	 */
	private Map<Integer, List<ServiceFunctionChainPolicy>> policiesOfSf = null;

	/**
	 * A map to store measured latency for all SFC policies. SFCPolicy -> Duplicated
	 * SF -> Network delay between "From" and duplicated SF in SFC. Assumed that a
	 * SFC includes a SF only once in the chain ( SF1->SF2->SF1 not allowed because
	 * of SF1 twice ). For example, Policy1 defines -- src:VM1 -> SF2 -> SF3 ->
	 * dst:VM4, and there are SF2-1, SF2-2 as duplicated SFs, then
	 * latencyMap.get(Policy1).get(SF2-1) stores the latency from VM1(source of SF2)
	 * to SF2-1.
	 */
	private Map<ServiceFunctionChainPolicy, Map<ServiceFunction, Double>> latencyMap = new HashMap<>();

	/**
	 * Preference list of duplicated SFs for the original SF (key) and the
	 * SFCpolicy.
	 */
	private Map<ServiceFunction, Map<ServiceFunctionChainPolicy, List<ServiceFunction>>> policyPrefSf = new HashMap<>();

	/**
	 * Counter for every SFs including duplicated SFs. Different from
	 * super.orgSfCounter which is only for the original SF.
	 */
	private Map<ServiceFunction, Long> sfCounter = new HashMap<>();

	/**
	 * A constant value for how many extra packets can be forwarded to the preferred
	 * SF for time-critical applications.
	 */
	...
	/**
	 * Select a SF among duplicated SFs for the SFC policy which has to go through
	 * the orgSF. If the orgSF has been duplicated, this method chooses and returns
	 * one of them. If no duplicated SFs exists, simply orgSF itself is returned.
	 * 
	 * @param orgSF  the original SF that might have duplicated SFs by auto-scale
	 *               method.
	 * @param policy the SFCPolicy enforcing to pass through the orgSF. It can be
	 *               used as a SF selection criteria.
	...
	protected ServiceFunction loadbalanceSF(ServiceFunction orgSF, ServiceFunctionChainPolicy policy) {
		...
		if (passingPolicies == null || passingPolicies.size() <= 1) {
			// Only this policy is passing through the SF. Use a normal load balancing
			// method.
	 ...
	 * This method is called after completion of creating a new VM (SF) in a data
	 * center. The function is Overrided in order to build latency map after the
	 * completion of SF duplication.
	 * 
	...
	private void buildSFPolicyMap() {
		...
		for (int sfId : policiesOfSf.keySet()) {
			...
			// Sort the policies based on the expected latency (short latency = first in
			// list = high priority)
