...
/**
 * This class defines a SFC policy set: (src, dst, srcPort, dstPort, proto) =>
 * ServiceFunctionChain. Note that the tuple of (srcPort, dstPort, proto) is
 * represented as a single Channel ID in CloudSimSDN.
 *
...
public class ServiceFunctionChainPolicy {
	...
	/**
	 * A list of SFs (VMs) that the matching packets (src,dst,flow) have to go
	 * through.
	 */
	private List<Integer> sfc;

	/**
	 * Allocated bandwidth for the SFC policy, dynamically changeable by auto-scale
	 * techniques.
	 */
	private long bandwidth;

	/**
	 * Initial bandwidth allocated for the SFC policy, we retain it because
	 * bandwidth fluctuate by auto-scale.
	 */
	private long initBandwidth;

	/**
	 * Separate bandwidth allocated for each network section (between one SF to the
	 * next SF).
	 */
	...
	/**
	 * End-to-end delay threshold for scale up (expected response time). If the
	 * measured time is exceeding this, SFs/BWs will scale up to meet the required
	 * response time.
	 */
	 ...
	 * map for separate monitoring of different virtual channels between SF. Key
	 * (Integer) : VM id of the source (from) Value (Long) : processed bytes sent
	 * from the VM (key)
	 */
	private Map<Integer, Long> bwMonitorAccumulatedNetworkBytes = new HashMap<Integer, Long>(); // fromVmId ->
																								// processedBytes
	 ...
	 * @param flowId
	 * @param sfc
	 * @param expectedTime for auto Scale up
	 */
	public ServiceFunctionChainPolicy(int srcId, int dstId, int flowId, List<Integer> sfc, double expectedTime) {
		this.srcId = srcId;
		this.dstId = dstId;
		this.flowId = flowId;
		this.sfc = sfc; // VM ID of SFs. VMs should be created with the assigned ID prior to creating a
						// policy
	...
	private void updateMonitoredDelayData() {
		...
		/*
		 * if(countToRemove != 0) System.err.println(this+"Timeout!"+countToRemove);
		 */
	...
	public double getActualAvailability() {
		actualAvailability = calculateActualAvailability();
	...
	public double calculateActualAvailability() {
		...
		for (int i : sfc) {
			try {
				tempAvailability = tempAvailability * ServiceFunction.findSFById(i).getAvailability();
			} catch (Exception e) {
	...
	public double getNeededAvailability() {
		return neededAvailability;
	}

	public void setNeededAvailability(double neededAvailability) {
		this.neededAvailability = neededAvailability;
	}
