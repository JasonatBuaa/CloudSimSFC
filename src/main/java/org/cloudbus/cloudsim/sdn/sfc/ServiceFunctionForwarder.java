...
/**
 * Network packet forwarding module to enforce SFC for a SFC policy. This module
 * checks every packet passing within a data center in a NOS. If the tuple of
 * (source, destination, flowId) is matching to the SFC policy, the packet is
 * forwarded to the series of SFs in SFC. Default forwarding rule: Round Robin
 * based on the number of packets. For example, 5th packet to the SF with 3
 * duplicated SFs will be forwarded to the 2nd one.
 *
...
public class ServiceFunctionForwarder {
	...
	// Jason: apply multiple entry points for any SFCs. i.e., Multi - Access Point

	/** NOS which will call this forwarder. One forwarder per NOS */
	protected NetworkOperatingSystem nos;

	/**
	 * SFC policies used by the forwarder. Every packet will be investigated if any
	 * policy in this table matches. Key (String): tuple of (src,dst,flowid) in
	 * String data Value (SFCPolicy): SFCPolicy object corresponds to the key.
	 */
	protected HashMap<String, ServiceFunctionChainPolicy> policyTable = new HashMap<String, ServiceFunctionChainPolicy>(); // getKey(srcVM,
																															// dstVM,
																															// flowID)
																															// ->
																															// ServiceFunctionChainPolicy

	/**
	 * A pool of SFs used for auto-scaling. Once auto-scale policy creates a
	 * duplicated SF for the original SF, it will be added here. Key: original SF
	 * Value: the list of duplicated SFs for the original SF (including the original
	 * SF)
	 */
	protected Map<ServiceFunction, List<ServiceFunction>> sfPool; // Service Function Pool for Auto-Scaling

	/**
	 * Counter for each SF (only original SF, not for duplicated SF). It counts how
	 * many packets are forwarded for the SF.
	 */
	protected Map<ServiceFunction, Long> orgSfCounter;

	/**
	 * A map to retrieve the original SF by using a duplicated SF. This is to find
	 * the original SF in multi-cloud environment.
	 */
	protected static Map<Integer, ServiceFunction> sfOriginalMap = new HashMap<Integer, ServiceFunction>(); // new extra
																											// SF -->
																											// original
																											// SF.
	...
	/**
	 * This function is only called when new sf creation failed in current DC.
	 * 
	 * @param newSf
	 */
	...
	private Packet encapsulatePacket(Packet orgPacket, ServiceFunctionChainPolicy policy) {
		...
		// Log.printLine(CloudSim.clock() + ": ServiceFunctionForwarder.enforceSFC(): "+
		// orgPacket + " going through the chain of " + sfc);
		...
		for (int i = sfc.size() - 1; i >= 0; i--) {
			int sfId = sfc.get(i);
			pkt = redirectRequest(policy, pkt, sfId); // 增加policy指定的sf
		}
		return pkt;
	}
	...
	private Packet redirectRequest(ServiceFunctionChainPolicy policy, Packet orgPacket, int sfId) {
		...
		if (sfNOS == null)
			sfNOS = this.nos;
		// Log.printLine("ServiceFunctionForwarder.redirectRequest: "+ sfNOS.getName() +
		// " for policy: "+policy);
		...
		// Create the former request (SourceVM -> SF), which includes the latter request
		// processing afterwards.
		Transmission srcToSf = createRequestActivityTransmission(srcVmId, sf.getId(), flowId, packetSize,
				encapsulatedReq, null); // Jason: Here we could change the sfc packetSize accordingly

		// Log.printLine(CloudSim.clock() + ":
		// ServiceFunctionForwarder.redirectRequest(): "+ orgPacket + ": redirected to:
		// " + orgSF + " (LB to "+sf+")");
	...
	private Processing createRequestActivityProcessing(int cloudletId, int vmId, int userId, long workloadSize) {
		...
		Cloudlet cloudlet = new Cloudlet(cloudletId, workloadSize, peNum, fileSize, outputSize, utilizationModel,
	...
	private Transmission createRequestActivityTransmission(int fromVmId, int toVmId, int flowId, long packetSize,
			Request payload, Packet orgPacket) {
	...
	public void addDuplicatedSF(ServiceFunction orgSf, ServiceFunction newSf) {
		...
		nos.addExtraVm(newSf, nos); // This function will create a VM and get a callback, which will call
									// this.processVmCreateDyanmicAck() at last.
	}
	...
	private long getDuplicatedPathBandwidth(ServiceFunctionChainPolicy policy, int fromId, int toId) {
		// Duplicated SF is managed by the forwarder. Policy class only contains the
		// master information of the policy.
	...
	public void redistributeDuplicatedPathBandwidthAllChain(ServiceFunction orgSf) {
		...
		// Update all policies bandwidth, as the number of duplicated SFs in the policy
		// is changed.
	...
	public void updateSFCMonitor(int fromId, int toId, int flowId, long processedBytes) {
		/*
		 * ServiceFunction orgSf; if((orgSf = sfOriginalMap.get(fromId)) != null) fromId
		 * = orgSf.getId();
		 * 
		 * if((orgSf = sfOriginalMap.get(toId)) != null) toId = orgSf.getId();
		 */
		...
		for (ServiceFunctionChainPolicy policy : getAllPolicies()) {
			...
			if (policy.getFlowId() == flowId) {
				...
				if (success) {
					// Log.printLine(CloudSim.clock() + ": ServiceFunctionForwarder.updateMonitor():
					// "+ policy + ", network_bytes=" + processedBytes);
				}
			}
		}
	}
