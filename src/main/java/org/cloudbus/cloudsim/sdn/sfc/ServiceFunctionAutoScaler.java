...
/**
 * This class defines SFC auto-scale policy. It auto-scales ServiceFunction in
 * SFCs. NOS periodically calls scaleSFC() function during the monitoring. It
 * detects overloaded/underloaded SF chains and scales up/down the chain. When
 * SLA violation is detected, it adds more MIPS to the SF or more bandwidth to
 * the network flow. If no more resource is available from physical machine, it
 * duplicates SF in another physical machine.
 *
...
public class ServiceFunctionAutoScaler {
	...
	/**
	 * Historical data of scaled MIPS of each SF(VM). It will be used for scale
	 * down.
	 */
	private Map<ServiceFunction, List<CPUPeMips>> vmMipsHistory = new HashMap<ServiceFunction, List<CPUPeMips>>();

	/**
	 * Historical data of scaled bandwidth for each SFC. It will be used for scale
	 * down.
	 */
	...
	/**
	 * Starts auto-scale policy defined in the class. This function can be called
	 * periodically or whenever auto-scale policy has to be invoked.
	 * 
	 * @return none
	 */
	public void scaleSFC() {
		...
		for (ServiceFunctionChainPolicy policy : sfForwarder.getAllPolicies()) {
			...
			if (endToEndDelay > policy.getDelayThresholdMax()) {
				...
				Log.printLine(CloudSim.clock() + ": ServiceFunctionAutoScaler.scaleSFC(): SLA violated: " + policy
						+ ", delay=" + endToEndDelay + "/" + policy.getDelayThresholdMax() + ", req=" + numRequests
						+ ", BW_util_all_chain=" + bwUtilAverage);
					...
					for (int i = 0; i < vmIds.size() - 1; i++) {
						...
						if (bwUtil > policy.getBwUtilThresholdMax()) {
							Log.printLine(CloudSim.clock()
									+ ": ServiceFunctionAutoScaler.scaleSFC(): Network is overloded!!! BW Util: "
									+ fromId + "->" + toId + ": " + bwUtil + "/" + policy.getBwUtilThresholdMax());
				...
				if (Configuration.SFC_AUTOSCALE_ENABLE_SCALE_DOWN_BW) {
					...
					if (isScaleDown) {
						/*
						 * Log.printLine(CloudSim.clock() +
						 * ": ServiceFunctionAutoScaler.scaleSFC(): Network is underutilized!!!" +
						 * policy+" overall util = "+policy.getAverageBwUtil()); for(int i=0; i <
						 * vmIds.size()-1; i++) { int fromId = vmIds.get(i); int toId = vmIds.get(i+1);
						 * double bwUtil = policy.getAverageBwUtil(fromId, toId);
						 * Log.printLine(CloudSim.clock() +
						 * ": ServiceFunctionAutoScaler.scaleSFC(): Network is underutilized!!! BW Util: "
						 * +fromId+"->"+toId+": " + bwUtil + "/" + policy.getBwUtilThresholdMin()); }
						 */
		...
		// Check if a VM or policy needs to be both scale UP and DOWN simultaneously
		// (error!)
		...
		for (ServiceFunction sf : vmsToScaleUp.keySet()) {
			...
			Log.printLine(CloudSim.clock() + ": ServiceFunctionAutoScaler.scaleSFC(): SF(" + sf
					+ ") is overloded!!! util =" + getVMCpuUtilization(sf));
		...
		for (ServiceFunction sf : vmsToScaleDown) {
			Log.printLine(CloudSim.clock() + ": ServiceFunctionAutoScaler.scaleSFC(): SF(" + sf
					+ ") is underutilized!!! util =" + getVMCpuUtilization(sf));
			vmScaleDown(sf);
		}
	}
	...
	private void vmScaleUp(ServiceFunction sf, Set<ServiceFunctionChainPolicy> overloadedPolicy) {
		if (Configuration.SFC_AUTOSCALE_ENABLE_VM_VERTICAL && increaseVmCapacity(sf)) {
			...
			Log.printLine(CloudSim.clock() + ": ServiceFunctionAutoScaler.vmScaleUp(): VM(" + sf
					+ ") capacity is increased to:" + sf.getNumberOfPes() + "*" + sf.getMips());
		} else {
	...
	private void bwScaleUp(ServiceFunctionChainPolicy policy) {
		...
		Log.printLine(CloudSim.clock() + ": ServiceFunctionAutoScaler.bwScaleUp(): BW update:" + policy + " (New BW:"
				+ newBw + ")");
	...
	private void vmScaleDown(ServiceFunction sf) {
		...
		if (newMips != null) {
			Log.printLine(CloudSim.clock() + ": ServiceFunctionAutoScaler.vmScaleDown(): Scale down SF:" + sf
					+ " -> new MIPS:" + newMips);
			...
			if (removed) {
				Log.printLine(
						CloudSim.clock() + ": ServiceFunctionAutoScaler.vmScaleDown(): One VM is deleted for SF:" + sf);
				sfForwarder.redistributeDuplicatedPathBandwidthAllChain(sf);
			}
		}
	}
	...
	private void bwScaleDown(ServiceFunctionChainPolicy policy) {
		...
		if (newBw != -1) {
			...
			Log.printLine(CloudSim.clock() + ": ServiceFunctionAutoScaler.bwScaleDown(): SFC:" + policy + " -> new BW:"
					+ newBw);
			changePolicyBandwidth(policy, newBw);
		}
	...
	private ServiceFunction duplicateSF(ServiceFunction orgSf, Set<ServiceFunctionChainPolicy> overloadedPolicy) {
		...
		// Jason: where does this new identical VM resides in???
		// It seems that it resides in the same host as the initial one???
		// How to ensure if the physical resources are sufficient for the scale out
		// operation???
		CloudletScheduler clSch = new CloudletSchedulerSpaceSharedMonitor(Configuration.TIME_OUT);
		ServiceFunction newSF = null;
		try {
			newSF = new ServiceFunction(SDNVm.getUniqueVmId(), orgSf.getUserId(), orgSf.getMips(),
					orgSf.getNumberOfPes(), orgSf.getRam(), orgSf.getBw(), orgSf.getSize(), orgSf.getVmm(), clSch,
	...
	/*
	 * private double getChannelUtilization(Channel ch) { double currentTime =
	 * CloudSim.clock(); double startTime = currentTime -
	 * Configuration.migrationTimeInterval; return
	 * ch.getMonitoredUtilization(startTime, currentTime); }
	 */
	...
	private long getSFCBandwidthStepUp(ServiceFunctionChainPolicy policy) {
		// long orgBw = nos.getRequestedBandwidth(policy.getSrcId(), policy.getDstId(),
		// policy.getFlowId());
