 ...
 *
 * This is logical channel. One physical link (class Link) can hold more than
 * one logical channels (class Channel). Channel is directional. It is one way.
 * 
 * @author Jungmin Son
...
public class Channel {
	...
	public Channel(int chId, int srcId, int dstId, List<Node> nodes, List<Link> links, double bandwidth, SDNVm srcVm,
			SDNVm dstVm) {
		this.chId = chId;
		this.srcId = srcId;
		this.dstId = dstId;
		...
		this.srcVm = srcVm;
		// this.dstVm = dstVm;
		packetScheduler.setTimeOut(Configuration.TIME_OUT); // If the packet is not successfull in 3 seconds, it will be
															// discarded.
	}
	...
	public double getAdjustedRequestedBandwidth() {
		...
		// Find the slowest link (lowest bw) among all links where this channel is
		// passing through
		for (int i = 0; i < nodes.size() - 1; i++) {
			Node from = nodes.get(i);
			Link link = links.get(i);
	...
	// Channel adjustment!
	private boolean allocateMoreAvailableBw = false; // if true, we allocate leftover BW to channels (will get more than
														// requested)
	...
	public boolean adjustDedicatedBandwidthAlongLink() {
		...
		if (factor < 1.0) {
			System.err
					.println("Channel.adjustDedicatedBandwidthAlongLink(): " + this + ": Cannot allocate requested Bw("
							+ this.getRequestedBandwidth() + "). Allocate only " + requestedBandwidth);
		}
	...
	public boolean changeBandwidth(double newBandwidth) {
		...
		if (this.allocatedBandwidth == Double.NEGATIVE_INFINITY
				|| this.allocatedBandwidth == Double.POSITIVE_INFINITY) {
	 ...
	 * @return true if any tranmission has completed in this time, false if none is
	 *         completed.
	 */
	/*
	 * public boolean updatePacketProcessing(){ double currentTime =
	 * CloudSim.clock(); double timeSpent = currentTime -
	 * this.previousTime;//NetworkOperatingSystem.round(currentTime -
	 * this.previousTime);
	 * 
	 * if(timeSpent <= 0 || inTransmission.size() == 0) return false; // Nothing
	 * changed
	 * 
	 * //update the amount of transmission long processedThisRound =
	 * Math.round(timeSpent*getAllocatedBandwidthPerTransmission()); long
	 * processedTotal = processedThisRound*inTransmission.size();
	 * 
	 * this.increaseProcessedBytes(processedTotal);
	 * 
	 * //update transmission table; remove finished transmission
	 * LinkedList<Transmission> completedTransmissions = new
	 * LinkedList<Transmission>(); for(Transmission transmission: inTransmission){
	 * transmission.addCompletedLength(processedThisRound);
	 * 
	 * if (transmission.isCompleted()){ completedTransmissions.add(transmission);
	 ...
	 * ": Channel.updatePacketProcessing() ("+this.toString()+"):Time spent:"
	 * +timeSpent+
	 * ", BW/host:"+getAllocatedBandwidthPerTransmission()+", Processed:"
	 * +processedThisRound);
	 * 
	 * if(completedTransmissions.isEmpty()) return false; // Nothing changed return
	 * true; }
	 * 
	 * // Estimated finish time of one transmission private double
	 * estimateFinishTime(Transmission t) { double bw =
	 * getAllocatedBandwidthPerTransmission();
	 * 
	 * if(bw == 0) { return Double.POSITIVE_INFINITY; }
	 * 
	 * double eft= (double)t.getSize()/bw; return eft; }
	 * 
	 * // The earliest finish time among all transmissions in this channel public
	 * double nextFinishTime() { //now, predicts delay to next transmission
	 * completion double delay = Double.POSITIVE_INFINITY;
	 * 
	 * for (Transmission transmission:this.inTransmission){ double eft =
	 * estimateFinishTime(transmission); if (eft<delay) delay = eft; }
	 * 
	 * if(delay == Double.POSITIVE_INFINITY) { return delay; } else if(delay < 0) {
	 ...
	 */
	/*
	 * public double addTransmission(Transmission transmission){ if
	 * (this.inTransmission.isEmpty()) previousTime=CloudSim.clock();
	 * 
	 * this.inTransmission.add(transmission); double eft =
	 * packetScheduler.estimateFinishTime(transmission); return eft; }
	 */
	 ...
	 */
	/*
	 * public LinkedList<Transmission> getArrivedPackets(){ LinkedList<Transmission>
	 * returnList = new LinkedList<Transmission>();
	 * 
	 * if (!completed.isEmpty()){ returnList.addAll(completed); } completed.reset();
	 * 
	 * return returnList; }
	 */
	...
	public String toString() {
		return "Channel(" + this.srcId + "->" + this.dstId + "|" + this.chId + "): BW:" + allocatedBandwidth
				+ ", Transmissions:" + this.getActiveTransmissionNum();
	}
	...
	public boolean updatePacketProcessing() {
		...
		if (packetScheduler.getCompletedTransmission().isEmpty() && packetScheduler.getTimedOutTransmission().isEmpty())
			return false; // Nothing changed
		return true;
	}
}
