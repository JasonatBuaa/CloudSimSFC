...
public class VirtualNetworkMapper {
	...
	public boolean buildForwardingTable(int srcVm, int dstVm, int flowId) {
		...
		if (srchost == null || dsthost == null) {
			// System.err.println(CloudSim.clock() + ": " + getName() + ": Cannot find src
			// VM ("+srcVm+":"+srchost+") or dst VM ("+dstVm+":"+dsthost+")");
			return false;
		}
		...
		if (srchost.equals(dsthost)) {
			...
			Log.printLine(CloudSim.clock() + ": VMs are in different hosts:" + srchost + "(" + srcVm + ")->" + dsthost
					+ "(" + dstVm + ")");
	...
	// Jason: recursively build the forwarding table
	protected boolean buildForwardingTableRec(Node node, int srcVm, int dstVm, int flowId) {
		...
		if (nextLinkCandidates == null) {
			throw new RuntimeException("Cannot find next links for the flow:" + srcVm + "->" + dstVm + "(" + flowId
					+ ") for node=" + node + ", dest node=" + desthost);
		}
	...
	public boolean updateDynamicForwardingTableRec(Node node, int srcVm, int dstVm, int flowId, boolean isNewRoute) {
		...
		if (isNewRoute || !nextHop.equals(oldNextHop)) {
			...
			node.addVMRoute(srcVm, dstVm, flowId, nextHop);
			// Log.printLine(CloudSim.clock() + ": " + getName() + ": Updating VM route for
			// flow:"+srcVm+"->"+dstVm+"("+flowId+") From="+node+", Old="+oldNextHop+",
			// New="+nextHop);
	 ...
	 * @post $none
	 */
	public void buildNodesLinks(int src, int dst, int flowId, Node srcNode, List<Node> nodes, List<Link> links) {
