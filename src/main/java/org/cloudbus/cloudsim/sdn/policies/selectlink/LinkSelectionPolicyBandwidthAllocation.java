...
/**
 * Jason: Important!! Identical with {@link LinkSelectionPolicyFlowCapacity}
 * this policy counts the flows of a link, and select the least flow link.
 */
public class LinkSelectionPolicyBandwidthAllocation implements LinkSelectionPolicy {
	// Compare the total amount of dedicated bandwidth in links, and choose the
	// least full one.
	public Link selectLink(List<Link> links, int flowId, Node src, Node dest, Node prevNode) {
		...
		for (Link l : links) {
			...
			if (lCn < linkCn) {
				// Log.printLine(CloudSim.clock() + ": LinkSelectionPolicyFlowCapacity: Found
				// less crowded link: " + lBw + "<" + linkBw+". old="+l+", new="+link);
				Log.printLine(CloudSim.clock() + ": LinkSelectionPolicyFlowCapacity: Found less crowded link: " + lCn
						+ "<" + linkCn + ". old=" + l + ", new=" + link);
				link = l;
			}
		}
		return link;
