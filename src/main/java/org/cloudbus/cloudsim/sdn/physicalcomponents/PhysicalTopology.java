...
/**
 * Network connection maps including switches, hosts, and links between them in
 * physical layer.
 * 
 * @author Jungmin Son
...
public abstract class PhysicalTopology {
	public enum NodeType {
		Intercloud, Gateway, Core, Aggr, Edge, Host,
		// Local,
	};
	...
	public void addLink(Node fromNode, Node toNode, double latency) {
		...
		if (!nodesTable.containsKey(from) || !nodesTable.containsKey(to)) {
			throw new IllegalArgumentException("Unknown node on link:" + nodesTable.get(from).getAddress() + "->"
					+ nodesTable.get(to).getAddress());
		}

		if (linkTable.contains(fromNode.getAddress(), toNode.getAddress())) {
			throw new IllegalArgumentException(
					"Link added twice:" + fromNode.getAddress() + "->" + toNode.getAddress());
		}
