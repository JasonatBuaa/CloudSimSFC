...
public class Link {
	...
	public Link(Node highOrder, Node lowOrder, double latency, double bw) {
		...
		if (highOrder.getRank() > lowOrder.getRank()) {
			Node temp = highOrder;
			highOrder = lowOrder;
			lowOrder = temp;
		}
	...
	/*
	 * private double allocatedBandwidthDedicatedUp = 0; private double
	 * allocatedBandwidthDedicatedDown = 0;
	 * 
	 * private double getAllocatedBandwidthForDedicatedChannels(Node from) {
	 * if(this.isUplink(from)) return allocatedBandwidthDedicatedUp; else return
	 * allocatedBandwidthDedicatedDown; }
	 */
	...
	/*
	 * public double getFreeBandwidthForDedicatedChannel(Node from) { double bw =
	 * this.getBw(from); double dedicatedBw =
	 * getRequestedBandwidthForDedicatedChannels(from);
	 * 
	...
	public String toString() {
		return "Link:" + this.highOrder.toString() + " <-> " + this.lowOrder.toString() + ", upBW:" + upBW
				+ ", Latency:" + latency;
	}
